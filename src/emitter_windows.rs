...
use winapi::um::winuser::{
    INPUT, INPUT_KEYBOARD, KEYBDINPUT, KEYEVENTF_KEYUP, SendInput, VK_CONTROL, VK_LWIN, VK_MENU,
    VK_SHIFT,
};
...
pub fn send_key(keycode: u16, flags: u64) {
    ...
    if flags & 0x00040000 != 0 {
        inputs.push(make_key_input(VK_CONTROL as u16, false));
    }
    if flags & 0x00020000 != 0 {
        inputs.push(make_key_input(VK_SHIFT as u16, false));
    }
    if flags & 0x00080000 != 0 {
        inputs.push(make_key_input(VK_MENU as u16, false));
    }
    if flags & 0x00100000 != 0 {
        inputs.push(make_key_input(VK_LWIN as u16, false));
    }
    ...
    if flags & 0x00100000 != 0 {
        inputs.push(make_key_input(VK_LWIN as u16, true));
    }
    if flags & 0x00080000 != 0 {
        inputs.push(make_key_input(VK_MENU as u16, true));
    }
    if flags & 0x00020000 != 0 {
        inputs.push(make_key_input(VK_SHIFT as u16, true));
    }
    if flags & 0x00040000 != 0 {
        inputs.push(make_key_input(VK_CONTROL as u16, true));
    }
